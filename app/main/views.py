...
@main.route('/captcha')
def captcha(captcha_type='png'):
    chars = Captcha.get_chars(4)
    cap = Captcha(chars)
    buf = StringIO.StringIO()
    code_img = cap.get_captcha()
    code_img.save(buf,'PNG',quality=70)
    buf_str = buf.getvalue()
    response = make_response(buf_str)
    response.headers['Content-Type'] = 'image/png'
    cap.save_captcha("first")
    return response
...
def index():
    ...
    # pagination = Post.query.order_by(Post.timestamp.desc()).paginate(page, per_page=current_app.config['FLASKY_POSTS_PER_PAGE'], error_out=True)
    show_followed = False
    ...
    if show_followed:
        ...
        query = Post.query
    pagination = query.order_by(Post.timestamp.desc()).paginate(page, per_page=current_app.config['FLASKY_POSTS_PER_PAGE'], error_out=True)
...
def follow(username):
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash('Oops , Invalid user.','danger')
        return redirect(url_for('.index'))
...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followers.paginate(page, per_page=current_app.config['FLASKY_FOLLOWERS_PER_PAGES'], error_out=True)
...
def followed_by(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followed.paginate(page, per_page=current_app.config['FLASKY_FOLLOWERS_PER_PAGES'], error_out=True)
