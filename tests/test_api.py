...
class APITestCase(FlaskyTestCase):
    ...
    def test_no_auth(self):
        ...
        response = self.client.post(url_for('api.new_post'), content_type='application/json')
    ...
    def test_user(self):
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u1 = User(email='john@example.com', username=unicode('john'), password='cat', confirmed=True, role=r)
        u2 = User(email='susan@example.com', username=unicode('susan'), password='dog', confirmed=True, role=r)
